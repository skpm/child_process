...
var Readable = require('@skpm/stream').Readable
var Writable = require('@skpm/stream').Writable
...
module.exports = function spawn(_command, _args, _options) {
  ...
  function onStdout(data) {
    ...
    console.log(data && data.length() && readingStdout)
    if (data && data.length() && readingStdout) {
      if (!result.stdout.push(Buffer.from(data))) {
        readingStdout = false
        task.standardOutput().fileHandleForReading().setReadabilityHandler(null)
      }
    }
  }
  function onStderr(data) {
    if (data && data.length() && readingStderr) {
      if (!result.stderr.push(Buffer.from(data))) {
        readingStderr = false
        task.standardError().fileHandleForReading().setReadabilityHandler(null)
      }
    }
  }
  ...
  result.sdtin = new Writable({
    write: function write(chunk, encoding, callback) {
      task.standardInput().fileHandleForWriting().writeData(chunk.toNSData())
      callback()
    },
